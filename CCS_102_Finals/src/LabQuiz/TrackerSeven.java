...
public class TrackerSeven {
    ...
    private static final String[] DEFAULT_RESOURCES = { 
        "Electricity", "Water", "Internet", "Lab Equipment" 
    };
    private static final int NUM_DEFAULT_RESOURCES = 4;

    /**
     * Main method - program entry point
     * Controls the overall program flow and execution sequence
     */
    ...
    /**
     * REQUIRED METHOD 1: inputData()
     * Accept and validate resource usage inputs from the user
     * Allows individual data entry for specific building and resource combinations
     */
...
//        System.out.println(">>> Tip: Enter 'back' at any time to return to menu.");
        ...
        while (true) {
            ...
            if (!shouldContinueDataEntry()) break;
        }
    }
    
    /**
     * REQUIRED METHOD 2: displayMatrix()
     * Display all data in formatted tabular form
    ...
    /**
     * REQUIRED METHOD 3: computeBuildingTotals()
     * Calculate total and average usage per building
     * Computes row-wise totals and averages for each building
     */
    ...
    /**
     * REQUIRED METHOD 4: computeResourceAverages()
     * Calculate average usage per resource type
     * Computes column-wise averages for each resource across all buildings
     */
    ...
    /**
     * REQUIRED METHOD 5: detectInefficiencies()
     * Identify any building exceeding average resource consumption
     * Flags buildings consuming more than 10% above campus average
     */
            ...
            if (buildingAverage != null) {
                ...
                if (percentAboveAverage > INEFFICIENCY_THRESHOLD) {
                    if (!foundInefficientBuildings) {
                        System.out.println("ALERT: BUILDINGS EXCEEDING " + INEFFICIENCY_THRESHOLD + "% ABOVE CAMPUS AVERAGE:");
                        foundInefficientBuildings = true;
                    }
                    System.out.printf("   [!] %s: Avg=%.2f (%.1f%% above campus avg)\n", 
                                    dailyData[buildingIndex][0], buildingAverage, percentAboveAverage);
                }
            }
        }
    ...
    /**
     * REQUIRED METHOD 6: findMostEfficientBuilding()
     * Determine which building has the lowest average resource consumption
     * Identifies top performer and optionally the least efficient building
     */
    public static void findMostEfficientBuilding() {
        ...
        if (mostEfficientIndex != -1) {
            System.out.println("TOP PERFORMER: " + dailyData[mostEfficientIndex][0] + 
                             " (Avg: " + String.format("%.2f", lowestConsumption) + ")");
            
            if (leastEfficientIndex != -1 && leastEfficientIndex != mostEfficientIndex) {
                System.out.println("NEEDS IMPROVEMENT: " + dailyData[leastEfficientIndex][0] + 
                                 " (Avg: " + String.format("%.2f", highestConsumption) + ")");
    ...
    /**
     * REQUIRED METHOD 7: generateSummaryReport()
     * Display and interpret all analytical findings
     * Provides comprehensive summary with strategic recommendations
     */
    ...
    private static void displayAnalyticalFindings() {
        ...
        if (mostEfficient != -1) {
            ...
            System.out.println(">>> Performance Gap: " + 
                             String.format("%.2f", (maxConsumption - minConsumption)) + " units");
        }
    }
    ...
    public static void setupDataStructures() {
        ...
        displaySuccessMessage("System initialized with " + buildingCount + " buildings, " + 
    ...
    public static void calculateCampusWideSummary() {
        ...
        dailyData[dailyAveragesRow][dailyAveragesCol] = buildingCount > 0 ? 
            String.format("%.2f", sumOfAverages / buildingCount) : "0.00";
    }
    ...
    public static void computePercentageDifferences() {
        Double campusAverage = calculateCampusAverageConsumption();
        if (campusAverage == null || campusAverage <= 0) return;
    ...
    public static void copyDailyDataToOverview() {
        ...
        overviewData[currentDay][dailyAveragesCol] = resourceCount > 0 ? 
            String.format("%.2f", dayTotalConsumption / resourceCount) : "0.00";
    ...
    public static void resetDailyDataForNextDay() {
        for (int rowIndex = 1; rowIndex < dailyData.length; rowIndex++) {
            for (int colIndex = 0; colIndex < dailyData[rowIndex].length; colIndex++) {
                if (rowIndex >= dailyTotalsRow && colIndex == 0) continue;
    ...
    public static void initializeArrayWithPlaceholders(String[][] targetArray, int summaryRow, String rowPrefix) {
        for (int rowIndex = 1; rowIndex < targetArray.length; rowIndex++) {
            for (int colIndex = 0; colIndex < targetArray[rowIndex].length; colIndex++) {
                if (rowIndex >= summaryRow && colIndex == 0) continue;
    ...
    public static void calculateDayWiseOverviewTotals() {
        for (int dayIndex = 1; dayIndex < overviewTotalsRow; dayIndex++) {
            ...
            overviewData[dayIndex][dailyAveragesCol] = validDataCount > 0 ? 
                String.format("%.2f", dayTotal / validDataCount) : "0.00";
        }
    }
    ...
    public static void calculateResourceWiseOverviewTotals() {
        for (int resourceIndex = 1; resourceIndex < overviewData[0].length; resourceIndex++) {
            if (resourceIndex == dailyAveragesCol || resourceIndex == dailyTotalsCol) continue;
            ...
            overviewData[overviewAveragesRow][resourceIndex] = validDataCount > 0 ? 
                String.format("%.2f", resourceTotal / validDataCount) : "0.00";
        }
    }
}