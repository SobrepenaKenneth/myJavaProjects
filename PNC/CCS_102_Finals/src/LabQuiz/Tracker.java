...
public class Tracker {
	...
	/*
	 * 
	 * XXX [0][column] are used as headers. XXX [row][0] are reserved for building
	 * names.
	 * 
	 */
	...
	public static void setArraySize() {
		...
		for (int r = 1; r < buildings.length; r++) {
			for (int c = 0; c < buildings[r].length; c++) {
				if (r >= totalsRow && c == 0)
					continue;
				if (c == 0)
					buildings[r][c] = Integer.toString(r);
				else
					buildings[r][c] = "-";
		...
		for (int r = 1; r < overview.length; r++) {
			for (int c = 0; c < overview[r].length; c++) {
				if (r >= overVTotalsRow && c == 0)
					continue;
				if (c == 0)
					overview[r][c] = Integer.toString(r);
				else
					overview[r][c] = "-";
		...
		System.out.println("> An Empty Data Set with " + row + " rows and " + column + " columns has been created."
	...
	public static void DailyTotalsAndAverages() {
		...
		for (int row = 1; row < totalsRow; row++) {
			...
			if (itemCount != 0)
				buildings[row][averageColumn] = Double.toString(total / itemCount);
			total = 0;
			itemCount = 0;
		}
		...
		for (int column = 1; column < buildings[0].length; column++) {
			...
			if (column != averageColumn) {
				...
				if (itemCount != 0)
					buildings[averageRow][column] = Double.toString(total / itemCount);
				else
					buildings[averageRow][column] = Double.toString(0);
			}
			total = 0;
			itemCount = 0;
		}
	...
	public static void OverviewTotalsAndAverages() {
		...
		for (int row = 1; row < overVTotalsRow; row++) {
			...
			if (itemCount != 0)
				overview[row][averageColumn] = Double.toString(total / itemCount);
			total = 0;
			itemCount = 0;
		}
		...
		for (int column = 1; column < overview[0].length; column++) {
			...
			if (column != averageColumn) {
				...
				if (itemCount != 0)
					overview[overVAverageRow][column] = Double.toString(total / itemCount);
				else
					overview[overVAverageRow][column] = Double.toString(0);
			}
			total = 0;
			itemCount = 0;
		}
	...
	public static void DetectInefficiencies() { // TODO: REQUIRES REVISION
		...
		for (int row = 1; row < totalsRow; row++) {
			...
			if (currentItem > columnAverage)
				System.out.print(buildings[row][0] + " ");
	...
	public static void findMostEfficientBuilding() { // TODO: REQUIRES REVISION
		...
		if (efficientIndex != 0)
			System.out.println(buildings[efficientIndex][0]);
		else
			System.out.println();
	...
	public static void FinaliseAndSave() {
		// Takes the Totals Column for the current set and records it in the overview
		// set.
		for (int column = 1; column < buildings[0].length; column++) {
