...
public class searchAnElementInThreeDimensional {
	public static void main(String[] args) {
		Scanner scan = new Scanner(System.in);
		int row, col, layer, search;
		
		System.out.println("=:> Search An Element in 3D Array <:=");
		...
		for (int layers = 0; layers < numbers.length; layers++) {
			for (int rows = 0; rows < numbers[layers].length; rows++) {
				for (int cols = 0; cols < numbers[layers][rows].length; cols++) {
					if (search == numbers[layers][rows][cols]) {
						System.out.println("Element found at Layer " + (layers + 1) + ", Row " + (rows + 1) + ", Column " + (cols + 1));
					}
				}
			}
		}
